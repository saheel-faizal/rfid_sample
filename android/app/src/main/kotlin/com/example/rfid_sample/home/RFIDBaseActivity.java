...
import com.zebra.demo.rfidreader.settings.SelectAntennaDialog;
...
public class RFIDBaseActivity implements Readers.RFIDReaderEventHandler,
    ...
    private boolean isInventoryFragment = false;
    ...
    public void onCreate(ActiveDeviceActivity mActivity) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            mActivity.registerReceiver(BroadcastReceiver, filter, RECEIVER_EXPORTED);
        }else{
            mActivity.registerReceiver(BroadcastReceiver, filter);
        }

        RFIDController.getInstance().clearAllInventoryData();
        if(mConnectedReader != null && mConnectedReader.getHostName() != null && mConnectedReader.getHostName().startsWith("MC33")) {
            Intent i = new Intent();
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            mActivity.registerReceiver(mReceiver, bluetoothFilter, RECEIVER_EXPORTED);
        }else{
...
//         PP+ Battery support
    ...
    private void enableBatterySaverModeChangeListner() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                mActivity.registerReceiver(mDeviceIdlereceiver, new IntentFilter(PowerManager.ACTION_DEVICE_IDLE_MODE_CHANGED),RECEIVER_EXPORTED);
            }else{
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                mActivity.registerReceiver(mIdlemodechangereciever, new IntentFilter(PowerManager.ACTION_POWER_SAVE_MODE_CHANGED), RECEIVER_EXPORTED);
            }else{
    ...
    private void LoadProfiles() {
        ...
        if ("RE40".equalsIgnoreCase(model)) {
            ...
            contentRE40.LoadDefaultProfiles(mActivity);
        }
    }
        ...
        if (mConnectedReader != null && mConnectedReader.isConnected()) {
            if (!mIsInventoryRunning) {
                ...
                if (inventoryBT != null) {
                    ...
                    if(isInventoryFragment){
                        inventoryBT.setContentDescription("stop_inventory");
                    }else{
                        inventoryBT.setContentDescription("stop_rapid_read");
                    }
                }
                                ...
                                new RfidListeners() {
                                    ...
                                    public void onFailure(String message) {
                                        ...
                                        if (inventoryBT != null) {
                                            ...
                                            if(isInventoryFragment){
                                                inventoryBT.setContentDescription("start_inventory");
                                            }else{
                                                inventoryBT.setContentDescription("start_rapid_read");
                                            }
                                        }
                                    }
                                }
            ...
            if (mConnectedReader != null && mConnectedReader.isConnected()) {
                if (Application.multiTagLocateTagListExist) {
                    ...
                    if (!Application.mIsMultiTagLocatingRunning  ) {
                        ...
                        v.setContentDescription("stop_multi_locate");
                        ...
                        v.setContentDescription("start_multi_locate");
    ...
    public synchronized void locationingButtonClicked(final View v) {
        ...
        if (locateTag != null && !isLocatingTag && !locateTag.isEmpty()) {
            ...
            if (btn_locate != null) {
                ...
                btn_locate.setContentDescription("stop_tag_locate");
            }
            ...
            if (btn_locate != null) {
                ...
                btn_locate.setContentDescription("start_tag_locate");
            }
...
//            ((AccessOperationsFragment) fragment).deviceConnected(device);
    ...
    public void ReaderDeviceDisConnected(ReaderDevice device) {
        ...
        if (fragment instanceof ReaderWifiSettingsFragment) {
            ((ReaderWifiSettingsFragment) fragment).deviceDisconnected();
        }
    ...
    * On factory reset clean up
    ...
    private ReaderDevice getReaderDevice(ReaderDevice readerdevice) {
        ...
        try {
            if( mConnectedReader.getHostName().startsWith("RFID")) {
    ...
    private void notificationFromGenericReader(RfidStatusEvents rfidStatusEvents) {
        ...
        if (rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.DISCONNECTION_EVENT) {
            ...
            Fragment wififragment =  mActivity.getCurrentFragment(SETTINGS_TAB);
            if(wififragment instanceof ReaderWifiSettingsFragment) {
                ((ReaderWifiSettingsFragment) wififragment).readWifiScanNotification(status, ssid);
            }else if (wififragment instanceof WifiFragment) {
                ((WifiFragment) wififragment).readWifiScanNotification(status, ssid);
            }

        } else if (rfidStatusEvents.StatusEventData.getStatusEventType() == STATUS_EVENT_TYPE.FIRMWARE_UPDATE_EVENT) {
            Log.d(TAG,"GTM UI Call back -> "+rfidStatusEvents.StatusEventData.FWEventData.m_status);
            int setTab = RFD_DEVICE_MODE == DEVICE_STD_MODE ? SCAN_TAB :SETTINGS_TAB;
            Fragment updateFwFragment = mActivity.getCurrentFragment(setTab);
            if(updateFwFragment instanceof UpdateFirmware )
                ((UpdateFirmware) updateFwFragment).updateFirmwareProgress(rfidStatusEvents.StatusEventData.FWEventData);
        }
    }
    ...
    public void eventWifiScanNotify(RfidWifiScanEvents rfidwifiscanevents) {
        ...
        Log.d(TAG, "eventWifiScanNotify: " + " ssid: "+ data.wifiscandata.getssid()
                + " macaddress:" + data.wifiscandata.getmacaddress()+ " key:" + data.wifiscandata.getProtocol() +"\n");
        Fragment fragment = mActivity.getCurrentFragment(SETTINGS_TAB);
        if(fragment instanceof ReaderWifiSettingsFragment){
    ...
    public void showAntennaSelection() {
        new SelectAntennaDialog().show(mActivity.getSupportFragmentManager(), (selectedAntennas)-> {
            RFIDController.getInstance().setSelectedAntennas(selectedAntennas);
        });
    }
